...
const images = [
  ...
  {
    url: "https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?q=80&w=2670&auto=format&fit=crop",
    ...
    <section id="gallery" className="py-24 bg-slate-50 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16 space-y-4">
          <h2 className="text-sm font-bold text-brand-cyan tracking-[0.4em] uppercase">Visual Journey</h2>
          <h3 className="text-5xl font-bold text-brand-navy uppercase tracking-tight">EXPERIENCE VILLA TIGA</h3>
        </div>
          ...
          {images.map((img, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-brand-navy/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8">
                <p className="text-brand-cyan text-[10px] font-bold uppercase tracking-[0.3em] mb-2">{img.title}</p>
                <p className="text-white text-xl font-medium">Breathtaking Architecture</p>
              </div>
            </div>
          ))}
        </div>
